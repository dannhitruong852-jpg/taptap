...
class T(unittest.TestCase):
    ...
    def test_accepts_presegmented_reviewed_rows_without_changing_text(self):
        with tempfile.TemporaryDirectory() as td:
            td=Path(td); bundle=td/'x.tgz'; root=td/'out'
            rows=[[1,'Alpha.','甲。','explain',[]],[2,'Beta?','乙？','explain',[]]]
            self._write_bundle(bundle,rows,{'sentence_count':2})
            materialize(bundle,root)
            doc=json.loads((root/'2011/text1.candidate.json').read_text(encoding='utf-8'))
            self.assertEqual(doc['article']['rows'],rows)
            self.assertEqual(''.join(r[1] for r in doc['article']['rows']),'Alpha.Beta?')
            self.assertEqual(''.join(r[2] for r in doc['article']['rows']),'甲。乙？')

    def test_rejects_one_row_review_that_claims_multiple_aligned_sentences(self):
        with tempfile.TemporaryDirectory() as td:
            td=Path(td); bundle=td/'x.tgz'; root=td/'out'
            self._write_bundle(
                bundle,
