...
class ProductionReadyTests(unittest.TestCase):
        ...
        for year in YEARS:
            ...
            for unit in UNITS:
                (year_dir / f'{unit}.candidate.json').write_text(json.dumps({
                    ...
                    'qa': {'sentence_count': 1, 'source_scope_verified': True},
                }, ensure_ascii=False))
    ...
    def test_missing_source_scope_verified_is_reported_as_candidate_error(self):
        td, root = self.fixture()
        self.addCleanup(td.cleanup)
        path = root / 'reports/content-freeze/2007/cloze.candidate.json'
        doc = json.loads(path.read_text())
        doc['qa'].pop('source_scope_verified')
        path.write_text(json.dumps(doc, ensure_ascii=False))
        result = self.run_checker(root)
        report = json.loads(result.stdout)
        self.assertIn('2007/cloze', report['candidate_errors'])
        self.assertFalse(report['ready'])
