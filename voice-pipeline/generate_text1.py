...
def encode_opus(input_wav, output_opus, rate):
    ...
    subprocess.run(['ffmpeg','-y','-i',str(input_wav),'-filter:a',f'atempo={rate:.3f}',
                    '-c:a','libopus','-b:a','48k',str(output_opus)],check=True,
                   stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)

def main():
    # Pure helpers stay importable without the heavyweight TTS runtime.
    import torch
    ...
    if len(segments) != 24:raise RuntimeError(f'expected 24 segments, got {len(segments)}')
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = ChatterboxTTS.from_pretrained(device=device)
    manifest = {'article':{'year':2002,'section':'Text 1'},'model_version':MODEL_VERSION,'segments':{}}
    for index, segment in enumerate(segments, start=1):
        ...
        wav = model.generate(segment['text'],audio_prompt_path=str(reference_path),
        ...
        fingerprint = generation_fingerprint(text=segment['text'],actor_id=actor_id,
            reference_pack_version=reference['reference_pack_version'],emotion=segment['emotion'],
            intensity=segment['intensity'],rate=segment['rate'],model_version=MODEL_VERSION)
        manifest['segments'][segment['id']]={'path':f"./audio/2002/text1/{segment['id']}.opus",
            'actor_id':actor_id,'emotion':segment['emotion'],'intensity':segment['intensity'],
            'rate':segment['rate'],'seed':seed,'fingerprint':fingerprint,'qa_status':'candidate'}
