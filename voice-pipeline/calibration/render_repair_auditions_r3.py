"""Render third-round targeted C v4 QA2 repair auditions."""
...
from calibration.repair_search_r3 import build_repair_r3_render_plan
from calibration.render_auditions import MODEL, sha256, wav_metrics


def stable_repair_r3_seed(actor_id: str, scene_id: str, variant: str) -> int:
    raw = hashlib.sha256(f"c-v4-repair-r3|{actor_id}|{scene_id}|{variant}".encode()).digest()
...
def render(profile_path: Path, script_path: Path, references_dir: Path, output: Path, device: str) -> Path:
    ...
    completed = []
    for item in build_repair_r3_render_plan(profile, script):
        ...
        seed = stable_repair_r3_seed(item["actor_id"], item["scene_id"], item["variant"])
    ...
    manifest = output / f"actor{profile['actor_id']}-repair-r3.json"
    manifest.write_text(
        json.dumps({
            "schema_version": "c-v4-repair-audition-3",
