"""Render fifth-round localized C v4 QA2 repair candidates for actors 03 and 15."""
...
from calibration.repair_search_r5 import build_repair_r5_render_plan
from calibration.render_auditions import MODEL, sha256, wav_metrics


def stable_repair_r5_seed(actor_id: str, scene_id: str, variant: str) -> int:
    raw = hashlib.sha256(f"c-v4-repair-r5|{actor_id}|{scene_id}|{variant}".encode()).digest()
...
def render(profile_path: Path, script_path: Path, references_dir: Path, output: Path, device: str) -> Path:
    ...
    completed = []
    for item in build_repair_r5_render_plan(profile, script):
        ...
        seed = stable_repair_r5_seed(item["actor_id"], item["scene_id"], item["variant"])
    ...
    manifest = output / f"actor{profile['actor_id']}-repair-r5.json"
    manifest.write_text(
        json.dumps({
            "schema_version": "c-v4-repair-audition-5",
