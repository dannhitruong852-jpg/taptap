"""Render actor15 fourth-round pronunciation/prosody repair auditions."""
...
from calibration.repair_search_r4_15 import build_actor15_r4_render_plan
from calibration.render_auditions import MODEL, sha256, wav_metrics


def stable_actor15_r4_seed(scene_id: str, variant: str, alias_id: str) -> int:
    raw = hashlib.sha256(f"c-v4-actor15-r4|{scene_id}|{variant}|{alias_id}".encode()).digest()
...
def render(profile_path: Path, script_path: Path, references_dir: Path, output: Path, device: str) -> Path:
    ...
    completed = []
    for item in build_actor15_r4_render_plan(profile, script):
        reference = references_dir / item["reference"]
        if not reference.is_file():
            raise FileNotFoundError(reference)
        stem = f"actor15-{item['scene_id']}-{item['variant']}"
        wav_path = output / f"{stem}.wav"
        opus_path = output / f"{stem}.opus"
        mp3_path = output / f"{stem}.mp3"
        seed = stable_actor15_r4_seed(item["scene_id"], item["variant"], item["alias_id"])
    ...
    manifest = output / "actor15-repair-r4.json"
    manifest.write_text(
        json.dumps({
            "schema_version": "c-v4-repair-audition-4-actor15",
            "actor_id": "15",
