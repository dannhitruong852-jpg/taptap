"""Render actor15 Round 9 candidates using fresh seeds at the Round 8 Z24 control point."""
...
from calibration.repair_search_r9_15 import build_actor15_r9_render_plan
from calibration.render_auditions import MODEL, sha256, wav_metrics


def stable_repair_r9_seed(actor_id: str, scene_id: str, variant: str) -> int:
    raw = hashlib.sha256(f"c-v4-repair-r9|{actor_id}|{scene_id}|{variant}".encode()).digest()
...
def render(profile_path: Path, script_path: Path, references_dir: Path, output: Path, device: str) -> Path:
    ...
    completed = []
    for item in build_actor15_r9_render_plan(profile, script):
        ...
        seed = stable_repair_r9_seed(item["actor_id"], item["scene_id"], item["variant"])
    ...
    manifest = output / "actor15-repair-r9.json"
    manifest.write_text(json.dumps({
        "schema_version": "c-v4-repair-audition-9",
        "actor_id": "15",
