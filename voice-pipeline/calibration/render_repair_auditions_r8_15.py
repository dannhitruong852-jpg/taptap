"""Render actor15 Round 8 bootstrap candidates while retaining independent identity evidence."""
...
from calibration.repair_search_r8_15 import build_actor15_r8_render_plan
from calibration.render_auditions import MODEL, sha256, wav_metrics


def stable_repair_r8_seed(actor_id: str, scene_id: str, variant: str) -> int:
    raw = hashlib.sha256(f"c-v4-repair-r8|{actor_id}|{scene_id}|{variant}".encode()).digest()
...
def render(profile_path: Path, script_path: Path, references_dir: Path, output: Path, device: str) -> Path:
    ...
    completed = []
    for item in build_actor15_r8_render_plan(profile, script):
        identity_reference = references_dir / item["reference"]
        prompt_reference = references_dir / item["prompt_reference"]
        if not identity_reference.is_file():
            raise FileNotFoundError(identity_reference)
        if not prompt_reference.is_file():
            raise FileNotFoundError(prompt_reference)
        ...
        seed = stable_repair_r8_seed(item["actor_id"], item["scene_id"], item["variant"])
        torch.manual_seed(seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(seed)
        audio = model.generate(
            item["synthesis_text"],
            audio_prompt_path=str(prompt_reference),
        ...
        record = {
            **item,
            ...
            "prompt_reference_sha256": sha256(prompt_reference),
    ...
    manifest = output / "actor15-repair-r8.json"
    manifest.write_text(json.dumps({
        "schema_version": "c-v4-repair-audition-8",
        "actor_id": "15",
