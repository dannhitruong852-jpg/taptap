"""C-mode v3 renderer for all six 2002 units using content-selected cast voices.
Each segment is conditioned on the same actor in an emotion-matched EARS reference.
No artificial silence or post-generation tempo manipulation is added.
"""
import argparse
import hashlib
import json
...
RENDER_VERSION='2002-c-v3-content-cast'
ARTICLES=('cloze','text1','text2','text3','text4','translation')
REFERENCE_DIR=ROOT/'voice-pipeline/references/2002-cast'
...
def reference_filename(segment):
    return f"actor{segment['actor_id']}-{segment.get('emotion','neutral')}.wav"
...
def render_fingerprint_v3(segment,reference_sha,seed):
...
def run_segments(segments,output,render_one,references,article,max_attempts=2,shard=0):
    ...
    manifest_path=output/f'v3-shard-{shard}.json'
    ...
    for segment in segments:
        sid=segment['id'];base_seed=int(hashlib.sha256(f'2002:v3:{article}:{sid}'.encode()).hexdigest()[:8],16)
        ref_sha=references[(segment['actor_id'],segment.get('emotion','neutral'))];cached=old['segments'].get(sid)
        if cached and any(cached.get('fingerprint')==render_fingerprint_v3(segment,ref_sha,base_seed+a) for a in range(max_attempts)) and cache_valid(cached,output):
            report['segments'][sid]=cached;report['cache_hits']+=1;continue
        for attempt in range(max_attempts):
            seed=base_seed+attempt
            try:
                result=render_one(segment,seed,output)
                files={f'v3-{sid}.{ext}':sha(output/f'v3-{sid}.{ext}') for ext in ('opus','mp3')}
                controls=performance_controls(segment)
                result.update({'path':f'./audio/2002/c-{article}/v3-{sid}.opus',
                               'mp3_path':f'./audio/2002/c-{article}/v3-{sid}.mp3',
                               'actor_id':segment['actor_id'],'emotion':segment['emotion'],
                               'intensity':segment['intensity'],'seed':seed,'attempts':attempt+1,
                               'reference_sha256':ref_sha,'fingerprint':render_fingerprint_v3(segment,ref_sha,seed),
                               'files':files,**controls})
                report['segments'][sid]=result
                print(f'v3 {article}/{sid} generated on attempt {attempt+1}',flush=True);break
            except Exception as exc:
                print(f'v3 {article}/{sid} attempt {attempt+1}: {exc}',flush=True)
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--article',required=True,choices=ARTICLES)
    ...
    required={(seg['actor_id'],seg.get('emotion','neutral')) for seg in source_segments}
    ref_paths={}
    for actor,emotion in required:
        path=REFERENCE_DIR/f'actor{actor}-{emotion}.wav'
        if not path.is_file():raise FileNotFoundError(f'Missing cast reference: {path}')
        ref_paths[(actor,emotion)]=path
    ref_hashes={(actor,emotion):sha(path) for (actor,emotion),path in ref_paths.items()}
    ...
    def render_one(segment,seed,directory):
        ...
        with torch.inference_mode():
            wav=model.generate(spoken_text(segment['text']),audio_prompt_path=str(ref_paths[(segment['actor_id'],segment.get('emotion','neutral'))]),
        ...
        sid=segment['id'];raw=Path(directory)/f'v3-{sid}.wav';ta.save(str(raw),wav,model.sr)
        filters='loudnorm=I=-19:TP=-2:LRA=11'
        for ext,codec,bitrate in [('opus','libopus','64k'),('mp3','libmp3lame','112k')]:
            subprocess.run(['ffmpeg','-v','error','-y','-i',str(raw),'-af',filters,'-ar','48000','-ac','1','-c:a',codec,'-b:a',bitrate,str(Path(directory)/f'v3-{sid}.{ext}')],check=True)
        raw.unlink()
        actual=float(subprocess.check_output(['ffprobe','-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1',str(Path(directory)/f'v3-{sid}.opus')]))
        return {'duration_seconds':round(actual,3),'raw_wpm':round(wpm,1),'clipped_fraction':clipped,
                'technical_qa':'passed','qa_status':'candidate','c_mode_version':'v3-content-cast'}
