"""Merge C v4 semantic/role chunks into final sentence assets for any exam year."""
...
ARTICLE_ORDER = {'cloze': 0, 'text1': 1, 'text2': 2, 'text3': 3, 'text4': 4, 'partb': 5, 'translation': 6}
...
def discover_articles(reader_root: Path, year: int) -> list[str]:
    names = [p.stem for p in (Path(reader_root) / f'content/{year}/c').glob('*.json')]
    return sorted(names, key=lambda name: (ARTICLE_ORDER.get(name, 99), name))


def merge_status(required_articles, articles: dict[str, dict]) -> str:
...
def _collect_segments(incoming: Path, article: str) -> dict[str, dict]:
    found = {}
    for path in Path(incoming).rglob('v4-shard-*.json'):
        data = load_json(path)
        if data.get('article') != article:
            continue
        entries = data.get('items') or data.get('segments') or {}
        for sid, entry in entries.items():
            ...
            if files.get(opus.name) != sha(opus) or files.get(mp3.name) != sha(mp3):
                continue
            candidate = {**entry, '_opus': opus, '_mp3': mp3}
            current = found.get(sid)
            if current and current.get('generation_fingerprint') != candidate.get('generation_fingerprint'):
                raise ValueError(f'conflicting rendered candidates for {article}/{sid}')
            found[sid] = candidate
    return found
...
def main() -> None:
    ...
    articles = discover_articles(reader_root, args.year)
    if not articles:
        raise ValueError(f'no canonical content found for {args.year}')
    summary = {
        'year': args.year, 'articles': {},
    ...
    summary['status'] = merge_status(articles, summary['articles'])
