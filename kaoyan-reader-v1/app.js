...
const audioPlayer=createAudioPlayer({
 ...
 onSegmentStart:segment=>{setPlaying(true,false);statusEl.textContent=`演员 ${segment.actor_id} · ${emotionLabels[segment.emotion]||segment.emotion}`;},
 onSentenceEnd:()=>{if(state.current<sentences.length-1){state.timer=window.setTimeout(()=>speak(state.current+1),260);}else{setPlaying(false,false);statusEl.textContent='这一篇读完了 ✓';}},
...
function speak(index,{scroll=true}={}){
 ...
 clearTimer();audioPlayer.stop();state.current=nextSentenceIndex(index,0,sentences.length);
 const sentence=sentences[state.current];updateActive(scroll);
 const queue=buildSentenceQueue(sentence,manifest).map(item=>({...item,path:!supportsOpus&&item.mp3_path?item.mp3_path:item.path}));
...
async function openArticle(entry){
 ...
 try{
  ...
  const audioCount=sentences.filter(s=>s.segments.every(x=>manifest.segments[x.id]?.path)).length;
  document.querySelector('#content-status').textContent=`${sentences.length} 句中英对照 · 音频 ${audioCount}/${sentences.length} 句可播放 · C模式待试听验收`;
  statusEl.textContent=audioCount===sentences.length?'静态音频 · 点句播放':'正文已就绪 · 音频生成中';
  history.replaceState(null,'',`#${entry.id}`);renderSentences();updateActive(false);applyPlayerVisibility(false);
