...
test('player contains previous play next and one fixed phrase-book control without speed UI',()=>{
  ...
  assert.equal((html.match(/id="phrase-book-open"/g)||[]).length,1);
  assert.match(html,/id="phrase-book-open"[^>]*>词群本<\/button>/);
  assert.doesNotMatch(html,/id="speed-control"|id="speed-value"|data-speed=/);
  assert.doesNotMatch(html,/id="replay"|id="speed-range"/);
});

test('X-style visual hierarchy uses two small pale controls one larger dark play control and a phrase-book pill',()=>{
